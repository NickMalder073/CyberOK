...
DWORD WINAPI ServerThread(LPVOID number) { //обработка потока клиента
    Packet packet;
    ...
    while (true) {
        ...
        if (packet == Pack) {
            ...
            if (responseMessage == "/exit") {
                cout << "client " << usernames[index] << " is lost!\n";
                break;
            }
            ...
            if (responseMessage == "/private") {
                ...
                cout << "friend's username: " << friendUsername << "\n";
            ...
            if (!isPrivate) { //если не приватный чат, то передаем всем участникам
                for (int i = 0; i < 10; i++)
                {
            ...
            else { //если приватный, то находим индекс клиента по имени и передаем только ему
                Packet messageType = Pack;
                int i = findIndexByUsername(friendUsername);
                cout << "friend's index: " << i << "\n";
                send(clients[i], (char*)&messageType, sizeof(Packet), NULL); //передача типа
                send(clients[i], (char*)&messageSize, sizeof(int), NULL); //передача размера
                send(clients[i], (char*)&response[0], messageSize, NULL); //передача самого сообщения 
            }
    ...
    clientsCounter--;
    cout << "number of clients: " << clientsCounter << "\n";
    return 0;
}

bool checkUsername(char* username, int index) { //проверка: занято ли имя
    string str1, str2;
    str1 = username;
...
int main()
{
    ...
    for (int i = 0; i < 10; i++) {
        ...
        if (newConnection == 0) {
            ...
            clientsCounter++;
            cout << "number of clients " << clientsCounter << endl;
