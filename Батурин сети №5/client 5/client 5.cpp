...
DWORD WINAPI ClientThread(LPVOID sock) { //функция потока для принятия сообщений
    SOCKET connection = ((SOCKET*)sock)[0];
    Packet packet;

    while (1) { //получения пакетов от сервера
...
int main() {
    ...
    string message;
    while (true) { //отправка данных серверу в основной поток
        getline(cin, message);
        ...
        if (message == "/private") {
            cout << "You enter in a private-mode, input username: (to exit type '/private-off') ";
